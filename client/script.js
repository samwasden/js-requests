...
// CODE HERE
function changeHelloButton(event) {
...
// CODE HERE
function changeItBack(event) {
    event.target.classList.remove("invert")
}

sayHelloButton.addEventListener("mouseout", changeItBack)
...
function queryRequest() {
    axios.get(`http://localhost:3000/query-test/?name=dave`).then((res) => {
...
it came back with a string that said you sent no queries

    2: Send more than 1 query on the URL -- what happened? 

it says, you sent more than one query then lists the queries
...
function createFood(event) {
    ...
    axios.post(`http://localhost:3000/food`, body).then((res) => {
        console.log(res.data)
        let paragraph = document.createElement("p")
        paragraph.textContent = res.data[res.data.length-1]
